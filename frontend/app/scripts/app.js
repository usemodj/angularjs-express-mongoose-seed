...
angular
  .module('frontendApp', [
    ...
    .run(['$rootScope', '$state','$stateParams', 'Authentications','gettextCatalog', function ($rootScope, $state, $stateParams, Authentications, gettextCatalog) {
        ...
        $rootScope.$on("$stateChangeStart", function(event, toState, toParams, fromState, fromParams) {
            if (!Authentications.authorize(toState.data.access)) {
                $rootScope.error = "Seems like you tried accessing a route you don't have access to...";
                event.preventDefault();

                if(fromState.url === '^') {
                    if(Authentications.isLoggedIn()) {
                        $state.go('user.home');
                    } else {
